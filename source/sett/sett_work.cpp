 ...
 ***************************************************************************************************
 *   File:          sett_work.cpp
 *   Description:   Модуль реализует работу с настройками flash. Алгоритмы универсальныдля всех
 *                  проектов
 *
 ***************************************************************************************************
...
/// Параметр, который всегда лежит в стуктуре последним
#define PARAM_ID_END PARAM_ID_VERS

/// максимальное количество BLOB параметров. Нужно для формирования отложенной записи
#define BLOB_BUF_SIZE FIFO_SIZE_16
 ...
 *              для возвращения данных по специфическим ID, если эти данные лежат в произвольной
 *              области памяти.
 *
 ...
 *
 *  @param[in]  after_update false - функция вызывается до изменения состояния flash памяти
 *              true - после.
 *
 *  @warning    Функция предназначена для перегрузки в высокоуровневом модуле, при необходимости
 */
...
static uint8_t _param_size_get(const param_header_t *hdr)
{
    ...
    switch (hdr->state)
    {
        case PARAM_DATA_BLOB  :
            size = sizeof(param_blob_t) ;
            break;
        case PARAM_DATA_1_BYTE:
            size = sizeof(param_uint8_t) ;
            break;
        case PARAM_DATA_2_BYTE:
            size = sizeof(param_uint16_t) ;
            break;
        case PARAM_DATA_4_BYTE:
            size = sizeof(param_uint32_t) ;
            break;
        case PARAM_DATA_8_BYTE:
            size = sizeof(param_uint64_t) ;
            break;
        default               :
            size = sizeof(param_trigger_t);
            break;
    }
 ...
 *  @details    Ищет в памяти нуждный ID. Просто обратиться к структуре нельзя, данные, лежащие
 *              во флеш могут не соответствовать текущей структуре
 *
 ...
 */
static param_header_t *_get_param_ptr(const __PACKED_STRUCT params_base_t *_addr,
                                      PARAM_ID_TYPE_t id)
{
 ...
 *  @details    BLOB данные начинаются сразу после структуры настроек, но структура может
 *              не соответствовать дефолтной на этапе обновления
 *
 ...
 *  @details    Используется для защиты от смены типаданных, т.к. это поле имеет два назначения -
 *              хранить триггерные параметры и задавать тип данных
 *
...
__INLINE static PARAM_TRIGGER_TYPE_t _value_to_state(uint64_t val)
{
    switch (val)
    {
        ...
        case PARAM_TRIGGER_OFF:
            return (PARAM_TRIGGER_TYPE_t)val;
        default               :
            return PARAM_TRIGGER_NA;
    }
};
...
static void _param_crc_write(bsp_flash_addr_t _addr)
{
    ...
    _flash_memcpy(&crc,
                  (uint8_t *)_addr + _flash_get_page_size(_addr) - sizeof(crc_t),
                  sizeof(crc_t));
};
...
static void _param_header_to_external(param_header_t *&_hdr, flash_param_external_t &_ext)
{
    if (_hdr != NULL)
    {
        ...
        else
        {
            ...
            {
                case PARAM_DATA_BLOB  :
                    _ext.len   = ((param_blob_t *)_hdr)->len ;
                    break;
                case PARAM_DATA_1_BYTE:
                    _ext.value = ((param_uint8_t *)_hdr)->data;
                    break;
                case PARAM_DATA_2_BYTE:
                    _ext.value = ((param_uint16_t *)_hdr)->data;
                    break;
                case PARAM_DATA_4_BYTE:
                    _ext.value = ((param_uint32_t *)_hdr)->data;
                    break;
                case PARAM_DATA_8_BYTE:
                    _ext.value = ((param_uint64_t *)_hdr)->data;
                    break;
                default:
                    _ext.type = 0;
                    _ext.value = _hdr->state;
                    break;
            }
        }
    }
    ...
    def_addr_(_def)
{
    if (false
        ...
        || _flash_get_page_size((bsp_flash_addr_t &)main_addr_) != _flash_get_page_size((
                    bsp_flash_addr_t &)mirr_addr_)
...
bool cpp_rw_params::must_be_updated(void)
{
    ...
    for (auto i = 0; i < PARAM_ID_COUNT; i++)
    {
        ...
        else if (   (uint32_t)param_hdr_dflt - (uint32_t)def_addr_ != (uint32_t)param_hdr_flash -
                    (uint32_t)main_addr_ // изменилось месторасположение параметров
                 || param_hdr_dflt->secure_flag !=
                    param_hdr_flash->secure_flag // изменился флаг прав доступа параметра
 ...
 *  @details    В целевую страницу записывается обновленная структура. Данные, по возможности
 *              берутся из структуры-источника. Если они конфликтуют с данными из дефолтной
 *              структуры, то их расположение, тип и права доступа меняются, подстраиваясь под
 *              дефолтную структуру. при этом данные, по возможности, сохраняются.
 *
 *  @param[in]  blob_save true - blob данные, при преобразовании в тип фиксированной длины
 *              сохраняются (обрезаются по длине), false - дефолтное значение
 */
bool cpp_rw_params::updating(const bool _blob_save)
{
    ...
    for (auto i = 0; i < i_max;)
    {
        ...
        param_header_t *param_hdr_source = _get_param_ptr(main_addr_,
                                           (PARAM_ID_TYPE_t)(param_hdr_dflt->id));
        ...
        if (param_hdr_dflt->state == PARAM_DATA_BLOB)
        {
            ...
            if (param_hdr_source == NULL)
            {
                ...
                blob_src_addr = (uint32_t)_blob_start_ptr(def_addr_) +
                                ((param_blob_t *)param_hdr_dflt)->shift;
            }
            ...
            else if (param_hdr_source->state == PARAM_DATA_BLOB)
            {
                ...
                blob_src_addr = (uint32_t)_blob_start_ptr(main_addr_) +
                                ((param_blob_t *)param_hdr_source)->shift;
            }
            ...
            if (blob_target.len > 0)
            {
                procrastinated_blob_t tmp =
                {
                    ...
                    .target_addr = (void *)((uint32_t)_blob_start_ptr(def_addr_) -
                                            (uint32_t)def_addr_ + (uint32_t)mirr_addr_ + blob_target.shift),
                    .len         = blob_target.len,
                };
        ...
        else
        {
            ...
            {
                case PARAM_DATA_1_BYTE:
                    tmp = ((param_uint8_t *)tmp_hdr)->data;
                    break;
                case PARAM_DATA_2_BYTE:
                    tmp = ((param_uint16_t *)tmp_hdr)->data;
                    break;
                case PARAM_DATA_4_BYTE:
                    tmp = ((param_uint32_t *)tmp_hdr)->data;
                    break;
                case PARAM_DATA_8_BYTE:
                    tmp = ((param_uint64_t *)tmp_hdr)->data;
                    break;
                case PARAM_DATA_BLOB:
                {
                    tmp = 0;
                    memcpy((void *)((uint32_t)_blob_start_ptr(main_addr_) +
                                    ((param_blob_t *)param_hdr_source)->shift),
                           &tmp,
                           (((param_blob_t *)param_hdr_source)->len < sizeof(tmp)) ?
                           ((param_blob_t *)param_hdr_source)->len : sizeof(tmp));
                    break;
                }
                default:
 ...
 *  @details    Восстанавливает страницу и в случае потери данных и добавляет дефолтные параметры,
 *              в более старые версии
 *
 *  @warning    Должна вызываться раньше чем любые другие функции работы с настройками,
 *              иначе целостность настроек не гарантируется
 */
...
bool cpp_rw_params::set(PARAM_ID_TYPE_t id, uint64_t value)
{
    ...
    if (result)
    {
        ...
        else
        {
            ...
            {
                case PARAM_DATA_BLOB  :
                    break; // Не пишем данные, если недопустимый тип
                case PARAM_DATA_1_BYTE:
                    val_not_eq = ((param_uint8_t *)param_header)->data != (uint8_t)value;
                    break;
                case PARAM_DATA_2_BYTE:
                    val_not_eq = ((param_uint16_t *)param_header)->data != (uint16_t)value;
                    break;
                case PARAM_DATA_4_BYTE:
                    val_not_eq = ((param_uint32_t *)param_header)->data != (uint32_t)value;
                    break;
                case PARAM_DATA_8_BYTE:
                    val_not_eq = ((param_uint64_t *)param_header)->data != (uint64_t)value;
                    break;
                default               :
                    val_not_eq = param_header->state != _value_to_state(value);
                    break;
            }
            ...
            for (uint32_t i = 0; curr_header->id != PARAM_ID_END;)
            {
                ...
                else
                {
                    ...
                    if (curr_header->id != id)
                    {
                        ...
                        data_ptr = (void *)((uint8_t *)blob_ptr_ +
                                            ((param_blob_t *)curr_header)->shift);
                    }
                    else
                    {
                        curr_blob.len = len;
                    ...
                    if (((uint32_t)blob_ptr_ + curr_blob.shift + curr_blob.len + sizeof(crc_t))
                        > (uint32_t)mirr_addr_ +
                        (uint32_t)_flash_get_page_size((bsp_flash_addr_t &)mirr_addr_))
                    {
                        // Если мы оказались здесь, значит, мы пишем что-то зело невпихуемое
                        // в страницу памяти
                        header = NULL;
                        break;
                    }
                    ...
                    {
                        // Не записываем сами данные, т.к. запись в память должна идти
                        // последовательно, иначе можно похерить невыровненные по словам данные
                        procrastinated_blob_t tmp =
                        {
                            .source_addr = data_ptr,
                            .target_addr = (uint8_t *)mirr_addr_ + (uint32_t)blob_ptr_ -
                            (uint32_t)main_addr_ + curr_blob.shift,
                            .len         = curr_blob.len,
                        };
                        // Не проверяем на переполнение, достаточно проверки в param_updating
                        FIFO_ADD(*blob_buf, tmp);
                    }
                }
            }
 ...
 *
 *  @return     true - запись прошла успешно, false - неверный тип параметра с данным ID,
 *              нет параметра с таким ID, данные не влезают в страницу настроек
 */
...
bool cpp_rw_params::clr(PARAM_ID_TYPE_t _id)
{
    ...
    if (result)
    {
        switch (curr->state)
        {
            ...
            case PARAM_DATA_1_BYTE:
                result = set(_id, ((param_uint8_t *)def)->data);
                break;
            case PARAM_DATA_2_BYTE:
                result = set(_id, ((param_uint16_t *)def)->data);
                break;
            case PARAM_DATA_4_BYTE:
                result = set(_id, ((param_uint32_t *)def)->data);
                break;
            case PARAM_DATA_8_BYTE:
                result = set(_id, ((param_uint64_t *)def)->data);
                break;
            default               :
                result = set(_id, def->state);
        }
    }
