...
/* History:
 *  custom
 *    Fixed CANx_MessageRead for CAN3 driver
 *  Version 1.16
...
#endif
#if defined RTE_CAN1_STB
    Enable_GPIO_Clock          (RTE_CAN1_STB_PORT);
    GPIO_InitStruct.Pin       = RTE_CAN1_STB_PIN;
    GPIO_InitStruct.Mode      = GPIO_MODE_OUTPUT_OD;
    GPIO_InitStruct.Pull      = GPIO_PULLUP;
    GPIO_InitStruct.Speed     = MX_CAN1_TX_GPIO_Speed;
    HAL_GPIO_Init              (RTE_CAN1_STB_PORT, &GPIO_InitStruct);
    HAL_GPIO_WritePin          (RTE_CAN1_STB_PORT, RTE_CAN1_STB_PIN, RTE_CAN1_STB_ACTIVE);
#endif
#else
    hcan1.Instance = CAN1;
#endif
...
#endif
#if defined RTE_CAN2_STB
    Enable_GPIO_Clock          (RTE_CAN2_STB_PORT);
    GPIO_InitStruct.Pin       = RTE_CAN2_STB_PIN;
    GPIO_InitStruct.Mode      = GPIO_MODE_OUTPUT_OD;
    GPIO_InitStruct.Pull      = GPIO_PULLUP;
    GPIO_InitStruct.Speed     = MX_CAN2_TX_GPIO_Speed;
    HAL_GPIO_Init              (RTE_CAN2_STB_PORT, &GPIO_InitStruct);
    HAL_GPIO_WritePin          (RTE_CAN2_STB_PORT, RTE_CAN2_STB_PIN, RTE_CAN2_STB_ACTIVE);
#endif
#else
    hcan2.Instance = CAN2;
#endif
...
#endif
#if defined RTE_CAN3_STB
    Enable_GPIO_Clock          (RTE_CAN3_STB_PORT);
    GPIO_InitStruct.Pin       = RTE_CAN3_STB_PIN;
    GPIO_InitStruct.Mode      = GPIO_MODE_OUTPUT_OD;
    GPIO_InitStruct.Pull      = GPIO_PULLUP;
    GPIO_InitStruct.Speed     = MX_CAN3_TX_GPIO_Speed;
    HAL_GPIO_Init              (RTE_CAN3_STB_PORT, &GPIO_InitStruct);
    HAL_GPIO_WritePin          (RTE_CAN3_STB_PORT, RTE_CAN3_STB_PIN, RTE_CAN3_STB_ACTIVE);
#endif
#else
    hcan3.Instance = CAN3;
#endif
...
{
    int32_t result = CANx_SetMode (mode, 0U);
#if defined RTE_CAN1_STB
    if (result == ARM_DRIVER_OK)
    {
        switch (mode)
        {
            case ARM_CAN_MODE_NORMAL:
            case ARM_CAN_MODE_LOOPBACK_EXTERNAL:
                HAL_GPIO_WritePin(RTE_CAN1_STB_PORT, RTE_CAN1_STB_PIN, RTE_CAN1_STB_ACTIVE ^ 1);
                break;
            default:
                HAL_GPIO_WritePin(RTE_CAN1_STB_PORT, RTE_CAN1_STB_PIN, RTE_CAN1_STB_ACTIVE);
        }
...
{
    int32_t result = CANx_SetMode (mode, 1U);
#if defined RTE_CAN2_STB
    if (result == ARM_DRIVER_OK)
    {
        switch (mode)
        {
            case ARM_CAN_MODE_NORMAL:
            case ARM_CAN_MODE_LOOPBACK_EXTERNAL:
                HAL_GPIO_WritePin(RTE_CAN2_STB_PORT, RTE_CAN2_STB_PIN, RTE_CAN2_STB_ACTIVE ^ 1);
                break;
            default:
                HAL_GPIO_WritePin(RTE_CAN2_STB_PORT, RTE_CAN2_STB_PIN, RTE_CAN2_STB_ACTIVE);
        }
...
{
    int32_t result = CANx_SetMode (mode, 2U);
#if defined RTE_CAN3_STB
    if (result == ARM_DRIVER_OK)
    {
        switch (mode)
        {
            case ARM_CAN_MODE_NORMAL:
            case ARM_CAN_MODE_LOOPBACK_EXTERNAL:
                HAL_GPIO_WritePin(RTE_CAN3_STB_PORT, RTE_CAN3_STB_PIN, RTE_CAN3_STB_ACTIVE ^ 1);
                break;
            default:
                HAL_GPIO_WritePin(RTE_CAN3_STB_PORT, RTE_CAN3_STB_PIN, RTE_CAN3_STB_ACTIVE);
        }
